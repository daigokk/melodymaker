...
int main() {
    Note a_part[] = {
		//波形タイプ, {周波数1(Hz), 周波数2(Hz),周波数3(Hz), 周波数4(Hz),周波数5(Hz), 周波数6(Hz)}, 持続時間(秒)
    ...
    Note* song[] = { a_part, b_part, c_part };
    int songSizes[] = {
        sizeof(a_part) / sizeof(Note),
        sizeof(b_part) / sizeof(Note),
        sizeof(c_part) / sizeof(Note)
    };
